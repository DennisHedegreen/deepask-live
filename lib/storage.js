...
const DEFAULT_SURVEY_ID = "public-data-possibilities";
const MAX_RESPONSES_PER_SURVEY = 1000;
const REACTION_PARTICIPANTS_KEY = "__reactionParticipants";
...
const defaultSurvey = {
  ...
  updated_at: "2026-07-14T20:22:07.000Z"
};
...
function normaliseSurvey(input) {
  ...
  return {
    ...
    subtitle: String(input.subtitle || "DeepAsk public-data survey").trim(),
    intro: String(input.intro || defaultSurvey.intro).trim(),
    mode: input.mode === "advanced" ? "advanced" : "simple",
    followup_limit: Math.max(1, Math.min(3, Number(input.followup_limit ?? 3))),
...
export function createReactionToken() {
  return randomUUID();
}
...
export async function saveResponse(workpack) {
  return withFileWrite(responsesPath, async () => {
    ...
    const savedWorkpack = {
      ...workpack,
      reaction_token: workpack.reaction_token || createReactionToken(),
...
export async function deleteSurveyParticipantData(surveyId) {
  const targetSurveyId = String(surveyId || "").trim();
  if (!targetSurveyId) throw new Error("survey_id is required");

  const removedResponses = await withFileWrite(responsesPath, async () => {
    await ensureStorage();
    const responses = await getResponses();
    const retained = responses.filter(
      (response) => String(response.survey_id || DEFAULT_SURVEY_ID) !== targetSurveyId
    );
    await writeJsonAtomic(responsesPath, retained);
    return responses.length - retained.length;
  });

  const removedReactions = await withFileWrite(reactionsPath, async () => {
    await ensureReactionsStorage();
    const reactions = await getMindHiveReactions();
    const scoped = reactions[targetSurveyId] || {};
    const count = Object.entries(scoped)
      .filter(([key]) => key !== REACTION_PARTICIPANTS_KEY)
      .reduce(
        (total, [, statement]) =>
          total +
          Object.values(statement || {}).reduce(
            (statementTotal, value) => statementTotal + Number(value || 0),
            0
          ),
        0
      );
    const { [targetSurveyId]: _removed, ...retained } = reactions;
    await writeJsonAtomic(reactionsPath, retained);
    return count;
  });

  return { surveyId: targetSurveyId, removedResponses, removedReactions };
}
...
export async function saveMindHiveReaction(surveyId, statementId, reactionType, participantToken) {
  return withFileWrite(reactionsPath, async () => {
    await ensureReactionsStorage();
    const reactions = await getMindHiveReactions();
    const scopedReactions = reactions[surveyId] || {};
    const tokenHash = hashReactionToken(participantToken);
    const participantReactions = scopedReactions[REACTION_PARTICIPANTS_KEY] || {};
    const participantKey = `${statementId}:${reactionType}`;
    const participantReactionKeys = participantReactions[tokenHash] || [];

    if (participantReactionKeys.includes(participantKey)) {
      return scopedReactionsWithCurrent(scopedReactions, statementId);
    }

    const current = scopedReactions[statementId] || {};
    reactions[surveyId] = {
      ...scopedReactions,
      [REACTION_PARTICIPANTS_KEY]: {
        ...participantReactions,
        [tokenHash]: [...participantReactionKeys, participantKey]
      },
      [statementId]: {
        ...current,
...
export function reactionsForSurvey(allReactions, surveyId) {
  if (allReactions?.[surveyId]) return stripReactionMetadata(allReactions[surveyId]);
  ...
  return legacyLooksFlat ? stripReactionMetadata(allReactions || {}) : {};
}
...
function scopedReactionsWithCurrent(scopedReactions, statementId) {
  return scopedReactions[statementId] || {};
}

function stripReactionMetadata(reactions) {
  const { [REACTION_PARTICIPANTS_KEY]: _participants, ...publicReactions } = reactions || {};
