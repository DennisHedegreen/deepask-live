import seededPatterns from "../fixtures/public-data-synthetic-patterns.json";
...
function buildStatement(pattern, documents, savedReactions) {
  const seedEvidence = new Set(pattern.seedEvidenceResponseIds || []);
  const related = documents.filter(
    (document) =>
      document.synthetic
        ? seedEvidence.has(document.id)
        : matchesPattern(document.text, pattern.keywords)
  );
  ...
  return {
    ...
    responseCount,
    evidenceResponseIds: related.map((document) => document.id),
    seedEvidenceResponseIds: [...seedEvidence],
    reactions: {
      ...DEFAULT_REACTIONS,
...
function responseToDocument(response) {
  ...
  const questionSummaries = Array.isArray(summary.question_summaries)
    ...
    : [];
  const participantAnswerText = Array.isArray(response.questions)
    ? response.questions
        .map((question) => {
  ...
  return {
    ...
    synthetic: response.synthetic === true || response.response_origin === "synthetic",
    text: [
      summary.main_theme,
...
function buildNextQuestions(statements) {
  ...
  if (statements.some((statement) => statement.id === "traceable-reproducible-ai-evidence")) {
    questions.push("How should participants challenge or correct a misleading pattern?");
  }

  if (statements.some((statement) => statement.id === "missing-people-and-missing-data-visible")) {
    questions.push("Which missing groups or places should the data gap warning name explicitly?");
  }

  if (statements.some((statement) => statement.id === "shared-standards-with-local-context")) {
